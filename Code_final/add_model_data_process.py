...
for i in range(7):
    for j in id_list:
        ...
        if j == 1:
            # /home/shelley/Desktop/hucares/DL_final2/2022_0504_to_now_2/feature_plus_2_model
            Test_bit_csv = f'/home/shelley/Desktop/hucares/DL_final2/2022_05_24_to_now/add_model/final_test_pred_id_{j}_fold{i}.csv'
            ...
            plt.savefig(f'final_ID_{j}_fold{i}_test_bit_y_true.png')  
            plt.clf()
            plt.plot(test_bit_pred,'b')
            plt.savefig(f'final_ID={j}_fold{i}_test_bit_pred.png')  
            plt.clf()

        if j == 6:
            Test_eth_csv = f'/home/shelley/Desktop/hucares/DL_final2/2022_05_24_to_now/add_model/final_test_pred_id_{j}_fold{i}.csv'
            ...
            if i == 0:
                ...
                all_test_predict[f'test_eth_y_true'] = test_eth_y_true
                
            else:
                avg_bit_test = np.sum([avg_bit_test,test_bit_pred], axis=0)
            all_test_predict[f'test_pred_id_{j}_fold{i}'] = test_eth_pred
            ...
            plt.savefig(f'final_ID={j}_fold{i}_test_eth_y_true.png')  
            plt.clf()
            
            plt.plot(test_eth_pred, 'r')
            plt.savefig(f'final_ID={j}_fold{i}_test_eth_pred.png')  
            plt.clf()
